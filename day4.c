...
char* input = "Card   1: 44 22 11 15 37 50  3 90 60 34 | 35 60 76  3 21 84 45 52 15 72 13 31 90  6 37 44 34 53 68 22 50 38 67 11 55\n"
...
int part1(char* string) {
    ...
    printf("winning_count: %d\n", winning_count);
    printf("your_count: %d\n", your_count);
    int * winning = calloc(winning_count, sizeof(int));
    int * yours = calloc(your_count, sizeof(int));
    int i = 0;
    while(*string != '\0') {
        if (*string == 'C') {
            in_winning = true;
        ...
        if (isdigit(*string)) {
            if(!in_num) {
                if(in_winning) {
                    winning[i++] = strtol(string, NULL, 0);
                } else {
                    yours[i++] = strtol(string, NULL, 0);
                }
            }
            in_num = true;
        } else in_num = false;

        if (*string == '|') in_winning = i = false;

        if (*string == '\n') {
            printf("\n");
            int points = 0;
            for (int j = 0; j < winning_count; ++j) printf("%2d ", winning[j]);
            printf("\n");
            for (int k = 0; k < your_count; ++k)
                printf("%2d ", yours[k]);
            printf("\n");
            for (int j = 0; j < winning_count; ++j) {
                for (int k = 0; k < your_count; ++k) {
                    if (winning[j] == yours[k]) {
                        printf("%2d-%-2d,", winning[j], yours[k]);
            ...
            total += points;
            printf("points: %d\n", points);
            memset(winning, 0, sizeof(int) * winning_count);
            memset(yours, 0, sizeof(int) * your_count);
        }
        string++;
    }
    return total;
...
int part2(char* string) {
    ...
    int card_count = 0;
    tmp = string;
    while(*tmp != '\0') if(*(tmp++) == 'C') card_count++;
    printf("winning_count: %d\n", winning_count);
    printf("your_count: %d\n", your_count);
    int * winning = calloc(winning_count, sizeof(int));
    int * yours = calloc(your_count, sizeof(int));
    int * card_copies = malloc(card_count * sizeof(int));
    for (int i = 0; i < card_count; ++i) card_copies[i] = 1;
    int i = 0;
    int card_number = 0;
    while(*string != '\0') {
        if (*string == 'C') {
        ...
        if (*string == '\n') {
            ...
            for (int j = 0; j < winning_count; ++j) {
                for (int k = 0; k < your_count; ++k) {
                    if (winning[j] == yours[k]) count++;
                }
            }
            total += card_copies[card_number];
            for (int j = 0; (j < count) && (card_number + j + 1 < card_count); ++j) {
                card_copies[card_number + 1 + j] += card_copies[card_number];
            }
            card_number++;
        }
        string++;
