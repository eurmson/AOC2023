//
// Created by eurms on 12/19/2023.
//
#include <ncurses.h>
#include <string.h>
#include <malloc.h>

char * input = "..FL-77L|7FL--J.F.77.F--F7J7FF-JJ.F-7J77.LJ7.L77F7F.F7777FFJ---7-77.FF-|.7---L7FL.-J.L-7F-J77FL7FF----|----J.FLF-F7.F-F-LJ..-FJJ7.FJ-7J-FJFL\n"
...
int part1(char* string) {
    ...
    lines[0] = strtok(dup_str, "\n");
    int line_len = strlen(lines[0]);
    for (int i = 1; i < line_count; ++i) lines[i] = strtok(NULL, "\n");
    int * distances = calloc(line_count * line_len, sizeof(int));
    memset(distances, -1, line_count * line_len * sizeof(int));
    WINDOW * win = initscr();
    noecho();
    start_color();
    raw();
    termattrs();
    init_color(40,10,30, 1);
    init_pair(1, COLOR_WHITE, COLOR_BLACK);
    init_pair(2, COLOR_RED, COLOR_BLUE);
    init_pair(3, COLOR_WHITE, COLOR_BLUE);
    int x = getmaxx(win);
    int y = getmaxy(win);
    y = y < line_count? y:line_count;
    int offset = 0;
    ...
    for (int i = 0; i < line_count; ++i) {
        char * pos;
        if ((pos = strchr(lines[i], 'S'))){
            sx = pos - lines[i];
            sy = i;
            break;
        }
    }
    if (sy > 0 && (lines[sy-1][sx] == 'F' || lines[sy-1][sx] == '7' || lines[sy-1][sx] == '|')){
        if(ax) {
            bx = sx;
            by = sy-1;
        } else {
    ...
    if (sy < line_count-1 && (lines[sy+1][sx] == 'J' || lines[sy+1][sx] == 'L' || lines[sy+1][sx] == '|')){
        if(ax) {
            bx = sx;
            by = sy+1;
        } else {
    ...
    if (sx > 0 && (lines[sy][sx-1] == 'F' || lines[sy][sx-1] == 'L' || lines[sy][sx-1] == '-')){
        if(ax) {
            bx = sx-1;
            by = sy;
        } else {
    ...
    if (sx < line_len && (lines[sy][sx+1] == '7' || lines[sy][sx+1] == 'J' || lines[sy][sx+1] == '-')){
        if(ax) {
            bx = sx+1;
            by = sy;
        } else {
    ...
    distances[sy * line_len + sx] = 0;
    distances[ay * line_len + ax] = 1;


    for (long long distance = 2; ay != by || ax != bx; distance++) {
        switch (lines[ay][ax]) {
            case 'F': {
                if (distances[(ay+1) * line_len + ax] == -1) distances[++ay * line_len + ax] = distance;
                else distances[ay * line_len + ++ax] = distance;
                break;
            }
            case '7': {
                if (distances[(ay+1) * line_len + ax] == -1) distances[++ay * line_len + ax] = distance;
            ...
            case '-': {
                if (distances[ay * line_len + (ax - 1)] == -1) distances[ay * line_len + --ax] = distance;
                else distances[ay * line_len + ++ax] = distance;
                break;
            }
        }
    ...
    while (true) {
        ...
        for (int i = 0; i < y; ++i) {
            move(i, 0);
            printw("%s", lines[i + offset]);
            char * pos = 0;
            if ((pos = strchr(lines[i + offset], 'S'))) {
                attron(COLOR_PAIR(2));
                move(i, pos - lines[i + offset]);
                printw("S");
                attron(COLOR_PAIR(1));
            }
            attron(COLOR_PAIR(3));
            for (int j = 0; j < line_len; ++j) {
                if(distances[(i + offset) * line_len + j] != -1) {
                    move(i, j);
                    addch(lines[i + offset][j]);
                }
            }
            attron(COLOR_PAIR(1));
            move(0, 0);
        ...
        if(c == 'q') break;
        //Down
        if(a == 0x1B && b == 0x5B && c == 0x42 && offset + y < line_count) offset++;
        //Left
        if(a == 0x1B && b == 0x5B && c == 0x44) printw("Left ");
        //Right
        if(a == 0x1B && b == 0x5B && c == 0x43) printw("Right");
        //Up
        if(a == 0x1B && b == 0x5B && c == 0x41 && offset > 0) offset--;
        refresh();
    };
    endwin();
    return distances[by * line_len + bx]/2 + 1;
}
...
int part2(char* string) {
    ...
    int line_len = strlen(lines[0]);
    for (int i = 1; i < line_count; ++i) lines[i] = strtok(NULL, "\n");
    long long * distances = calloc(line_count * line_len, sizeof(long long));
    memset(distances, -1, line_count * line_len * sizeof(long long));
    WINDOW * win = initscr();
    noecho();
    start_color();
    raw();
    ...
    char * state = calloc(sizeof(char) , line_len * line_count);
    for (int i = 0; i < line_count; ++i) {
        char in = 0;
        char entry = 0;
        for (int j = 0; j < line_len; ++j) {
            if (distances[i * line_len + j] != -1) {
                switch (lines[i][j]) {
                    case 'F':case 'L': {
                        entry = lines[i][j];
                        break;
...
//    lines[sy][sx] =
    ...
    while (true) {
        ...
        for (int i = 0; i < y; ++i) {
            ...
            attron(COLOR_PAIR(4));
            for (int j = 0; j < line_len; ++j) {
                if(state[(i + offset) * line_len + j] == -1) {
                    move(i, j);
                    addch(lines[i + offset][j]);
                }
            }
    ...
    endwin();
    int total = 0;
    for (int i = 0; i < line_len*line_count; ++i) total += state[i] > 0? 1:0;
    return total;
}
